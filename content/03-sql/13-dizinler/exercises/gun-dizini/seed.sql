...
-- Ileri Seviye eki: 20 000 satirlik bir olay tablosu.
--
-- Dizinlerin etkisi on satirlik tablolarda gorunmuyor. Satirlar rakamlarin
-- capraz carpimindan uretiliyor, yani her kurulumda birebir ayni. 20 000
-- satir veritabanini varsayilan 16 MB'in icinde tutuyor.
...
n AS (
    SELECT a.v + 10 * b.v + 100 * c.v + 1000 * e.v + 10000 * f.v + 1 AS k
    FROM d a CROSS JOIN d b CROSS JOIN d c CROSS JOIN d e CROSS JOIN d f
)
INSERT INTO events (id, customer_id, product_id, event_type, session_code, created_at, amount)
SELECT k,
       1 + k % 6,
       1 + k % 12,
       CASE k % 4 WHEN 0 THEN N'view' WHEN 1 THEN N'click' WHEN 2 THEN N'cart' ELSE N'purchase' END,
       'S' + RIGHT('000000' + CAST(k AS VARCHAR(6)), 6),
       DATEADD(minute, k * 25, CAST('2025-01-01' AS DATETIME2(0))),
       CASE WHEN k % 4 = 3 THEN CAST(k % 500 AS DECIMAL(10,2)) + 0.99 END
FROM n
WHERE k <= 20000;
